...
.EXITCODES
    0 = READY / déjà sur 25H2
    1 = NOT CAPABLE
    2 = UNDETERMINED / erreur de contrôle
    3 = CAPABLE BUT BLOCKED (GPO / Safeguard Hold)
...
try {
    ...
    Write-Check "Windows actuel" "INFO" ("{0} {1} - Build {2} - {3}" -f $ProductName, $DisplayVersion, $FullBuild, $EditionID)
    ...
    Write-Check "Carte graphique" $Graphics.State $Graphics.Detail
    ...
    $PermanentBlock = ($OSBlockers.Count -gt 0) -or (-not $HardwareCapable) -or ($Graphics.Capable -eq $false)
